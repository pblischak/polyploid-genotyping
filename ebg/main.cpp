...
int main(int argc, char* argv[]){
  ...
  if(strcmp(argv[1], "hwe") != 0 && strcmp(argv[1], "diseq") != 0 && strcmp(argv[1], "alloSNP") != 0){
    std::cerr << "\nA valid model was not selected. The options are: hwe, diseq, and alloSNP.\n" << std::endl;
    mainUsage();
    exit(EXIT_FAILURE);
  } else {
  ...
  if(strcmp(selectedModel.c_str(), "hwe") == 0){
    ...
    std::cerr << "\nThe selected model (" << selectedModel << ") is not a valid option.\n"
              << "Please choose one of the following: hwe, diseq, alloSNP.\n" << std::endl;
  }
